...
impl SSA {
    pub fn parse<S: AsRef<str>>(content: S) -> Result<SSA, SSAError> {
        Self::parse_with_options(content)
    }

    pub fn parse_lenient<S: AsRef<str>>(s: S) -> Result<SSA, SSAError> {
        Self::parse_with_options(s)
    }

    /// Parses the given [String] into [SSA].
    pub fn parse_with_options<S: AsRef<str>>(content: S) -> Result<SSA, SSAError> {
        let mut blocks = Vec::new();
        for (i, line) in (1..).zip(content.as_ref().lines()) {
            match line.trim() {
